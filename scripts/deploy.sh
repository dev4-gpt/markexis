...
# Map: workflow JSON path → workflow ID
declare -A WORKFLOWS=(
...
# Step 1: SCP all files to /tmp on VPS
echo "--- Step 1: Uploading workflow files ---"
for filepath in "${!WORKFLOWS[@]}"; do
  if [[ ! -f "$filepath" ]]; then
...
# Step 2: Copy into container and import + activate all workflows
echo ""
echo "--- Step 2: Importing + activating in n8n container ---"
for filepath in "${!WORKFLOWS[@]}"; do
  if [[ ! -f "$filepath" ]]; then continue; fi
  fname=$(basename "$filepath")
  wfid="${WORKFLOWS[$filepath]}"

  ssh -i "$SSH_KEY" "$VPS" "
    docker cp /tmp/${fname} n8n-n8n-1:/tmp/${fname} &&
  ...
  " && echo "  ✓ $wfid ($fname)" || echo "  ✗ FAILED: $fname"
done

# Step 3: Restart n8n to register all webhooks
echo ""
echo "--- Step 3: Restarting n8n to register webhooks ---"
ssh -i "$SSH_KEY" "$VPS" "cd ${N8N_DIR} && docker compose restart n8n"
echo "  Waiting 25s for n8n to start..."
sleep 25

# Step 4: Smoke-test all webhooks
echo ""
echo "--- Step 4: Webhook smoke test ---"
