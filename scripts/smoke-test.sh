...
# Can run from project root or from scripts/

N8N="http://67.207.89.85:5678/webhook"

# Webhook paths → human label
declare -A WEBHOOKS=(
...
if [[ $FAIL -gt 0 ]]; then
  ...
  echo "  2. Re-run: bash scripts/deploy.sh"
  echo "  3. If still failing, SSH in and check: docker logs n8n-n8n-1 --tail=50"
  exit 1
fi
